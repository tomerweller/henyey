...
- **Transaction Execution**: **FAILING**. The `verify-execution` command (which re-executes transactions) produces a hash mismatch. Investigation reveals that `ContractData` entries generated by our execution engine are missing the `write_timestamp` extension fields found in the authoritative CDP data. This implies a deficiency in the Soroban host integration or state update logic for Protocol 25.
...
### Remaining Work
Investigate why the execution engine fails to update `ContractData` timestamps (and potentially other Soroban state metadata) during transaction execution.
