...
- States (lowercase): `backlog`, `ready-for-planning`, `ready-for-doing`, `in-review`, `done`, `blocked`
...
| `ready-for-doing` | `/do` | Implements (Mode A) or addresses review comments (Mode B); opens PR / re-requests review; advances to `in-review` |
...
Skip items where any check fails. Skip items whose status is `done` or `blocked`.

### Step 3 — Pick one issue

Order actionable items by:

1. **Close-WIP-first state priority** — descending: `in-review` > `ready-for-doing` > `ready-for-planning` > `backlog`. Reason: prevents PRs from rotting in review while fresh backlog items pile up.
