...
### Testing Gaps
- Upstream tx meta hash vector coverage (synthetic vectors exist; upstream baseline loader/format checks in place; create-account low-reserve + success + already-exists + not-enough-funds + native-liabilities plus txresults create-account normal + with-payment-after, manage-data base + native buying/selling liabilities + too-many-subentries, bump-sequence, begin-sponsoring success + precondition-v3, end-sponsoring not-sponsored, inflation not supported, change-trust issuer-does-not-exist + native-asset, and payment existing-account + no-destination + dest-amount-too-big + fee-equal/base-reserve (min balance, one stroop, one fee minus one, one fee, one fee plus one, two fees minus two/one, two fees) + fee-bigger/base-reserve (min balance, one stroop, one fee minus one, one fee, one fee plus one, two fees minus two/one, two fees) baselines now match; remaining per-operation baseline hash parity tests still pending).
