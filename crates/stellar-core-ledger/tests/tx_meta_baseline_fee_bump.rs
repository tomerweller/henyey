...
fn baseline_file() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../testdata/tx-meta-baseline-current/FeeBumpTransactionTests.json")
}
...
fn create_set_options_envelope(
    source: &SecretKey,
    op_source: &SecretKey,
    base_fee: u32,
    sequence: i64,
    ...
    let op_source_id: AccountId = (&op_source.public_key()).into();
    let operation = Operation {
        source_account: Some(muxed_from_account_id(&op_source_id)),
    ...
    let op_sig = sign_envelope(&envelope, op_source, network_id);
    if let TransactionEnvelope::Tx(ref mut env) = envelope {
        env.signatures = vec![source_sig, op_sig].try_into().unwrap();
    }
    envelope
}
...
fn create_fee_bump_envelope(
    ...
    let inner_tx = Transaction {
        source_account: MuxedAccount::Ed25519(Uint256(*inner_source.public_key().as_bytes())),
        fee: inner_fee,
        seq_num: SequenceNumber(sequence),
        cond: Preconditions::None,
        memo: stellar_xdr::curr::Memo::None,
    ...
    let mut inner_env = TransactionV1Envelope {
        tx: inner_tx,
        signatures: VecM::default(),
    };
...
fn account_seq(entries: &HashMap<Vec<u8>, LedgerEntry>, account_id: &AccountId) -> i64 {
    let entry = entries
...
#[test]
fn fee_bump_inner_tx_failure_tx_level_meta_matches_baseline() {
    seed(12345).expect("seed short hash");
    let expected = load_baseline_hashes(
        "fee bump transactions|protocol version 25|apply|inner transaction fails, transaction level",
    );
    ...
    let set_options = create_set_options_envelope(
        &fee_source_secret,
        &root_secret,
        base_fee,
        fee_source_seq + 1,
...
#[test]
fn fee_bump_inner_tx_failure_op_level_meta_matches_baseline() {
    seed(12345).expect("seed short hash");
    let expected = load_baseline_hashes(
        "fee bump transactions|protocol version 25|apply|inner transaction fails, operation level",
    );
...
#[test]
fn fee_bump_validity_valid_meta_matches_baseline() {
    seed(12345).expect("seed short hash");
    let expected = load_baseline_hashes("fee bump transactions|protocol version 25|validity|valid");
    assert_eq!(expected.len(), 1);
