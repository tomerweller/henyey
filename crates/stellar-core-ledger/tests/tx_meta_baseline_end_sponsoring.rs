...
fn baseline_file() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../testdata/tx-meta-baseline-current/EndSponsoringFutureReservesTests.json")
}
...
#[test]
fn end_sponsoring_not_sponsored_baseline_matches() {
    seed(12345).expect("seed short hash");
    let expected =
        load_baseline_hashes("confirm and clear sponsor|protocol version 25|not sponsored");
