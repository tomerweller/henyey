...
use stellar_xdr::curr::{
    AccountEntry, AccountEntryExt, AccountId, AlphaNum4, AssetCode4, ChangeTrustAsset,
    ChangeTrustOp, ChangeTrustResult, LedgerEntry, LedgerEntryData, LedgerEntryExt, LedgerHeader,
    LedgerKey, LedgerKeyAccount, MuxedAccount, Operation, OperationBody, OperationResultTr,
    Preconditions, PublicKey, SequenceNumber, Signature as XdrSignature, SignatureHint, String32,
    Thresholds, TimePoint, Transaction, TransactionEnvelope, TransactionExt, TransactionMeta,
    TransactionResultMetaV1, TransactionV1Envelope, VecM, WriteXdr, Limits,
};
use stellar_core_ledger::execution::{ExecutionFailure, TransactionExecutionResult};
...
fn baseline_file() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../testdata/tx-meta-baseline-current/ChangeTrustTests.json")
}
...
fn execute_and_apply_with_result(
    ...
    envelope: TransactionEnvelope,
) -> (TransactionMeta, TransactionExecutionResult) {
    ...
    let (results, _tx_results, tx_result_metas, _id_pool) = execute_transaction_set(
        &snapshot,
        &[(envelope, None)],
        header.ledger_seq,
    ...
    let result = results.first().expect("tx result").clone();
...
fn setup_change_trust_world() -> ChangeTrustWorld {
    ...
    let gateway_account_id: AccountId = (&gateway_secret.public_key()).into();

    let base_fee = 100u32;
    let base_reserve = 100_000_000u32;
    let total_coins = 1_000_000_000_000_000_000i64;
    let min_balance2 = 4i64 * base_reserve as i64;
    ...
    let _ = execute_and_apply_with_result(
        &mut entries,
        &header,
        network_id,
        base_fee,
...
#[test]
fn change_trust_issuer_does_not_exist_new_trust_line_matches_baseline() {
    seed(12345).expect("seed short hash");
    let expected =
        load_baseline_hashes("change trust|protocol version 25|issuer does not exist|new trust line");
    assert_eq!(expected.len(), 1);

    let base_fee = 100u32;
    let base_reserve = 100_000_000u32;
    let mut world = setup_change_trust_world();

    let issuer_secret = secret_from_name("non-existing");
    let issuer: AccountId = (&issuer_secret.public_key()).into();
    ...
    let op_result = result.operation_results.first().expect("op result");
    assert!(matches!(
        op_result,
        stellar_xdr::curr::OperationResult::OpInner(OperationResultTr::ChangeTrust(
            ChangeTrustResult::NoIssuer
        ))
    ));
...
#[test]
fn change_trust_native_asset_matches_baseline() {
    seed(12345).expect("seed short hash");
    let expected =
        load_baseline_hashes("change trust|protocol version 25|trustline on native asset");
    assert_eq!(expected.len(), 1);
    ...
    let asset = ChangeTrustAsset::Native;
    let gateway_seq = account_seq(&world.entries, &world.gateway_account_id);
    let tx = change_trust_envelope(
        &world.gateway_secret,
        asset,
        i64::MAX - 1,
        base_fee,
        gateway_seq + 1,
