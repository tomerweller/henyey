...
/// Record hot archive restored entries that the host did NOT emit storage changes for.
///
/// When a TX restores entries from the hot archive but only reads them (no modification,
/// no TTL extension), the Soroban host does NOT emit storage changes for those entries.
/// Without this function, those entries would never be recorded in the ledger delta,
/// causing subsequent stages/clusters to fail to find them via prior_stage_entries.
///
/// Parity: stellar-core InvokeHostFunctionOpFrame::handleArchivedEntry unconditionally calls
/// `mOpState.upsertEntry(lk, le, ...)` and `mOpState.addHotArchiveRestore(lk, ...)` for every
/// hot archive restoration, regardless of whether the host subsequently modifies the entry.
///
/// This function creates the entry + TTL in the delta as INIT (since they are being moved
...
fn record_hot_archive_read_only_restores(
    ...
    for (key, (entry, restored_live_until)) in restored_entries {
        // Skip if already recorded in delta (host emitted a storage change for this key)
        if key_already_created_in_delta(state.delta(), key) {
            tracing::debug!(
                key_type = ?std::mem::discriminant(key),
                "Hot archive read-only restore: already in delta, skipping"
            );
            continue;
        }
        ...
        tracing::info!(
            key_type = ?std::mem::discriminant(key),
            restored_live_until,
            "Hot archive read-only restore: recording entry in delta (host did not emit change)"
        );

        // Create the entry in the delta as INIT (hot archive → live bucket list).
        match &entry.data {
            LedgerEntryData::ContractData(cd) => {
            ...
            _ => {
                tracing::warn!(
                    key_type = ?std::mem::discriminant(key),
                    "Hot archive read-only restore: unexpected entry type, skipping"
                );
                continue;
            }
        }

        // Create the TTL entry as INIT with the restored live_until.
        let key_hash = compute_key_hash(key);
        let ttl_key_hash_str = format!("{:?}", &key_hash);
        if !ttl_already_created_in_delta(state.delta(), &key_hash) {
            let ttl = TtlEntry {
            ...
            tracing::debug!(
                key_hash = ttl_key_hash_str,
                live_until = restored_live_until,
                "Hot archive read-only restore: created TTL entry in delta"
            );
        }
    }
}
...
mod tests {
    ...
    /// Regression test: hot archive restored entries that the host only reads (no storage change
    /// emitted) must still be recorded in the ledger delta as INIT entries.
    ///
    /// Bug: When a TX restores entries from hot archive but the host only reads them (does not
    /// modify or extend TTL), the `storage_changes` filter (`should_include = is_modification ||
    /// is_deletion || ttl_extended`) excluded those entries. `apply_soroban_storage_changes` was
    /// never called for them, so they were not recorded in the delta. In a parallel execution
    /// context, subsequent stage/cluster executors loaded `prior_stage_entries = delta.current_entries()`
    /// BEFORE the stage ran. The restored entries were absent, so the next stage's executor did
    /// not see them via `load_entry`. When the next stage TX had the same entry in its footprint
    /// and checked the hot archive, it found the entry still marked HOT_ARCHIVE_ARCHIVED (because
    /// the prior-stage restoration was never committed to the live BL), and incorrectly
    /// re-restored it — adding duplicate INIT entries to the live bucket list and causing a hash
    /// mismatch.
    ///
    /// Fix: `record_hot_archive_read_only_restores` explicitly creates the entry + TTL in the
    /// delta as INIT when the host did not emit a storage change, matching stellar-core's
    /// `handleArchivedEntry` which unconditionally calls `mOpState.upsertEntry(lk, le, ...)`.
    ///
    /// Observed at testnet ledger 60269153.
    #[test]
    fn test_hot_archive_read_only_restore_recorded_in_delta() {
        use sha2::{Digest, Sha256};
        use std::collections::HashMap;

        // Create a ContractCode entry to simulate a hot-archive-restored entry
        // that the host only read (no storage change emitted).
        let code_hash = Hash([0xABu8; 32]);
        let contract_code = ContractCodeEntry {
            ext: ContractCodeEntryExt::V0,
            hash: code_hash.clone(),
            code: BytesM::try_from(vec![1u8, 2u8, 3u8]).unwrap(),
        };
        let restored_entry = LedgerEntry {
            last_modified_ledger_seq: 50_000_000,
            data: LedgerEntryData::ContractCode(contract_code.clone()),
            ext: LedgerEntryExt::V0,
        };
        let contract_key = LedgerKey::ContractCode(LedgerKeyContractCode {
            hash: code_hash.clone(),
        });
        let restored_live_until: u32 = 60_342_752; // simulated restored TTL

        // Create a fresh state with no entries
        let mut state = LedgerStateManager::new(5_000_000, 60_269_153);

        // Verify: entry is NOT in state or delta before the fix is applied
        assert!(
            state.get_contract_code(&code_hash).is_none(),
            "Entry should not be in state before restoration"
        );
        assert!(
            !key_already_created_in_delta(state.delta(), &contract_key),
            "Entry should not be in delta before restoration"
        );

        // Simulate what happens BEFORE the fix: apply_soroban_storage_changes is called
        // with empty changes (the host only read the restored entry, no storage change emitted)
        // and hot_archive_restored_keys contains the key.
        // This is the BUGGY path — the entry never gets recorded.
        let hot_archive_restored_keys = {
            let mut s = std::collections::HashSet::new();
            s.insert(contract_key.clone());
            s
        };
        apply_soroban_storage_changes(
            &mut state,
            &[], // empty — host emitted no changes for this key
            &stellar_xdr::curr::LedgerFootprint {
                read_only: VecM::default(),
                read_write: vec![contract_key.clone()].try_into().unwrap(),
            },
            &hot_archive_restored_keys,
        );

        // After apply_soroban_storage_changes with no changes, the entry is STILL not in delta.
        // This demonstrates the bug: without the fix, the entry would be missing.
        assert!(
            !key_already_created_in_delta(state.delta(), &contract_key),
            "BUG REPRODUCED: entry not in delta after apply_soroban_storage_changes with empty changes"
        );
        ...
        // After the fix: the entry MUST be in the delta as a CREATED (INIT) entry
        assert!(
            key_already_created_in_delta(state.delta(), &contract_key),
            "Entry should be in delta as INIT after record_hot_archive_read_only_restores"
        );

        // Verify TTL entry is also created
        let key_bytes = contract_key
            .to_xdr(stellar_xdr::curr::Limits::none())
            .unwrap();
        let key_hash = Hash(Sha256::digest(&key_bytes).into());
        assert!(
            ttl_already_created_in_delta(state.delta(), &key_hash),
            "TTL entry should be in delta as INIT after record_hot_archive_read_only_restores"
        );

        // Verify the TTL value is correct
        let ttl = state.get_ttl(&key_hash);
        assert_eq!(
            ttl.map(|t| t.live_until_ledger_seq),
            Some(restored_live_until),
            "TTL live_until should be the restored value"
        );

        // Verify the contract code entry is accessible in state
        let code = state.get_contract_code(&code_hash);
        assert!(
            code.is_some(),
            "ContractCode should be accessible in state after recording"
        );
    }

    /// Regression test: record_hot_archive_read_only_restores must NOT duplicate entries
    /// that were already recorded by apply_soroban_storage_changes (host emitted a change).
    #[test]
    fn test_hot_archive_read_only_restore_skips_already_created_entries() {
        use std::collections::HashMap;

        let code_hash = Hash([0xCDu8; 32]);
        let contract_code = ContractCodeEntry {
            ext: ContractCodeEntryExt::V0,
            hash: code_hash.clone(),
            code: BytesM::try_from(vec![4u8, 5u8, 6u8]).unwrap(),
        };
        let contract_key = LedgerKey::ContractCode(LedgerKeyContractCode {
            hash: code_hash.clone(),
        });
        ...
        let restored_live_until: u32 = 60_342_752;

        let mut state = LedgerStateManager::new(5_000_000, 60_269_153);

        // Simulate the host DID emit a storage change (host modified the entry).
        // apply_soroban_storage_changes would have created it.
        let hot_archive_restored_keys = {
            let mut s = std::collections::HashSet::new();
            s.insert(contract_key.clone());
            s
        ...
        // Entry should be in delta after apply_soroban_storage_changes
        assert!(
            key_already_created_in_delta(state.delta(), &contract_key),
            "Entry should be in delta after apply_soroban_storage_changes with changes"
        );

        let created_count_before = state.delta().created_entries().len();

        // Now call record_hot_archive_read_only_restores — it should skip this entry
        // because it was already created by apply_soroban_storage_changes.
        let mut restored_entries: HashMap<LedgerKey, (LedgerEntry, u32)> = HashMap::new();
        restored_entries.insert(contract_key.clone(), (entry.clone(), restored_live_until));
        record_hot_archive_read_only_restores(&mut state, &restored_entries);

        let created_count_after = state.delta().created_entries().len();

        // No additional entries should have been created
        assert_eq!(
