...
async fn cmd_verify_execution(
    ...
    use std::sync::Arc;
    use stellar_core_bucket::{BucketList, BucketManager, HasNextState, HotArchiveBucketList};
    use stellar_core_common::Hash256;
    ...
    let (_network_id, network_name, is_mainnet) = if config.network.passphrase.contains("Test") {
        (stellar_core_common::NetworkId::testnet(), "testnet", false)
    } else {
        (stellar_core_common::NetworkId::mainnet(), "mainnet", true)
    };
                ...
                if all_match {
                    ...
                    if !header_matches && tx_result_matches {
                        ...
                        // Also dump expected final TX entries from CDP meta
                        {
                            ...
                            let bl_snapshot = stellar_core_bucket::BucketListSnapshot::new(&bl, result.header.clone());
                            drop(bl);
                            let mut diffs = 0;
                            let mut missing = 0;
