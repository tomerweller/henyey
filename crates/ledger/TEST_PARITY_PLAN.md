...
stellar-core: `stellar-core/src/ledger/test/` (6 files, ~42 top-level test cases)
...
| `LiabilitiesTests` | Split: `lib.rs` (reserves) + `henyey-tx/src/state.rs` |
...
| 4.18 | `LedgerTxnEntry move assignment` | N/A | **NOT APPLICABLE** | stellar-core move semantics test - Rust handles this via ownership |
...
| # | stellar-core Test | Rust Equivalent | Status | Notes |
|---|----------|-----------------|--------|-------|
| 4.27 | `InMemoryLedgerTxn simulate buckets` | N/A | **NOT APPLICABLE** | stellar-core in-memory mode for test simulation - Rust architecture is already in-memory |
| 4.28 | `InMemoryLedgerTxn getOffersByAccountAndAsset` | `state.rs:test_account_asset_index_*` | **COVERED** | Covered by OfferIndex tests |
| 4.29 | `InMemoryLedgerTxn getPoolShareTrustLinesByAccountAndAsset` | N/A | **MISSING** | Pool share trust line queries by account. May live in henyey-tx |
...
| 4.31 | `InMemoryLedgerTxn filtering` | N/A | **NOT APPLICABLE** | Internal stellar-core filtering of entry types for in-memory mode |
...
| # | stellar-core Test | Rust Equivalent | Status | Notes |
|---|----------|-----------------|--------|-------|
| 5.1 | `liabilities` / `add account selling liabilities` - various balance/reserve/sponsorship scenarios | `lib.rs:test_available_to_send` (minimal) | **MISSING** | stellar-core tests dozens of scenarios with different initBalance, initSellingLiabilities, initNumSubEntries, initNumSponsoring/Sponsored. Rust has one basic test |
...
Excluding NOT APPLICABLE tests (architecture-specific to stellar-core):
...
| ID | Test | Status | Rust Location |
|----|------|--------|---------------|
| P3-1 | Pool share trust line queries | **DONE** | `henyey-bucket/tests/test_pool_share_query.rs` (6 tests) |
...
| # | stellar-core Test | Status | Notes |
|---|----------|--------|-------|
