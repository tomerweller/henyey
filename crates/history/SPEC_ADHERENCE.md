...
- **henyey mechanism (two-window design)**:
  - **Window 1 (pre-final-persist):** `persist_bucket_list_snapshot()` (`catchup/mod.rs:488`) and `persist_header_only()` (`catchup/mod.rs:526`) write checkpoint-keyed rows that are durable but non-authoritative for the startup restore path. Startup reads from `last_closed_ledger` / `HISTORY_ARCHIVE_STATE` (in `crates/app`), not from ahead-of-LCL rows. A crash here leaves orphaned rows overwritten by the next successful catchup. **Caveat:** CLI readers (`publish_history`, `self_check`) still use `get_latest_ledger_seq()` → `MAX(ledgerseq)` and may observe ahead-of-LCL state; this is a known gap.
...
- **Status**: **Partial** (startup/catchup recovery path has semantic parity; CLI reader paths still anchor on `MAX(ledgerseq)` instead of durable LCL/HAS). The core recovery contract — startup never trusts interrupted catchup state — is preserved without a literal `REBUILD_FOR_OFFER_TABLE` flag because henyey has no SQL offer table (BucketListDB-only with in-memory offer index; see `crates/ledger/PARITY_STATUS.md`). Full parity requires hardening CLI readers to use LCL/HAS as their authority anchor.
