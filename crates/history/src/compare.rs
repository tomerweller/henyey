...
pub async fn compare_checkpoint(
    ...
    // --- HAS ---
    let local_has = local.checkpoint_has(checkpoint).await?;
    let ref_has = reference.checkpoint_has(checkpoint).await?;
    compare_has(&local_has, &ref_has, &mut mismatches);

    // --- Ledger headers ---
    let local_headers = local.ledger_headers(checkpoint).await?;
    let ref_headers = reference.ledger_headers(checkpoint).await?;
    compare_ledger_headers(&local_headers, &ref_headers, &mut mismatches);

    // --- Transactions ---
    let local_txs = local.transactions(checkpoint).await?;
    let ref_txs = reference.transactions(checkpoint).await?;
    compare_entries(&local_txs, &ref_txs, &mut mismatches);

    // --- Results ---
    let local_results = local.results(checkpoint).await?;
    let ref_results = reference.results(checkpoint).await?;
