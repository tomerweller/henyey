...
| C++ File | Description | Rust Equivalent |
|----------|-------------|-----------------|
...
| Retry mechanism | Yes | Yes | C++ has exponential backoff built-in |
...
| Batch work | Yes | No | C++ has iterator-based batch generation |
| Conditional work | Yes | No | C++ has monotonic condition gating |
...
- **`Work` trait**: Equivalent to C++ `BasicWork::onRun()`. Async work execution with outcome-based state transitions.
  - `name()` method matches C++ `getName()`
  - `run()` method matches C++ `onRun()` pure virtual

- **`WorkOutcome` enum**: Maps to C++ `BasicWork::State` return values:
  | Rust | C++ |
  |------|-----|
...
- **`WorkState` enum**: Simplified state machine compared to C++ `InternalState`:
  | Rust | C++ InternalState |
  |------|-------------------|
  | `Pending` | `PENDING` |
  | `Running` | `RUNNING` |
  | `Success` | `SUCCESS` |
...
- **`WorkScheduler`**: Core scheduler matching C++ `WorkScheduler` functionality:
...
- **`WorkSchedulerConfig`**:
  - `max_concurrency` - equivalent to C++ `MAX_CONCURRENT_SUBPROCESSES`
...
- **`WorkSequence`**: Linear chain builder (different from C++ design):
  - C++ `WorkSequence` is a `BasicWork` subclass managing internal work vector
...
- **`WorkWithCallback`**: Post-completion callback wrapper (different from C++ design):
  - C++ version wraps a callback function as the work itself
  - Rust version wraps another work item and calls callback after completion
  - Rust design better for instrumentation/logging; C++ design for inline logic
...
These C++ features rely on stellar-core's Application context and are handled differently in Rust:
...
C++ has 8 internal states with explicit transition validation:
```
...
Test file: `crates/stellar-core-work/tests/scheduler.rs`

## Future Considerations

If additional C++ parity is needed:
